...
# 靜態故障轉移資料庫 (當 2.5-flash 每天 20 次額度耗盡時，自動無縫接軌)
FALLBACK_QUIZ = [
...
def get_quiz_data():
    ...
    try:
        ...
        # 只要報錯 (包含 429 額度用盡)，直接丟出備用題庫，絕不讓系統崩潰
        return FALLBACK_QUIZ
    ...
    with col_r:
        ...
        if st.button("✨ 分析我的學習成效", key="btn_analysis"):
            with st.spinner("Gemini 正在為你診斷..."):
                try:
                    model = genai.GenerativeModel(MODEL_ID)
                    prompt = f"學生測驗得 {correct_count}/{total_q}。錯題紀錄：{mistakes_for_ai}。請寫一段溫暖鼓勵的學習成效分析。"
                    st.session_state.ai_analysis = model.generate_content(prompt).text
                except Exception:
                    st.session_state.ai_analysis = "⚠️ API 額度暫時耗盡。不用擔心，你的表現很棒！請針對左側的錯題解析進行複習，把基礎觀念打穩喔！"
        ...
        if st.button("✨ 產生研讀指南", key="btn_guide"):
            with st.spinner("Gemini 正在為你準備指南..."):
                try:
                    ...
                    st.session_state.ai_guide = "⚠️ API 額度暫時耗盡。建議你：1. 重新閱讀電解質定義。2. 默寫解離方程式。3. 確認電中性原則的核心精神。"
